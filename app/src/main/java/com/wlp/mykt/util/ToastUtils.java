...
public final class ToastUtils {
    ...
    private static int yOffset         = (int) (64 * Utils.mContext.getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) Utils.mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    /**
     * 显示短时吐司
     * 优先显示text；如果text为空，显示defaultResId
     *
     * @param text 文本
     * @param defaultResId 备用的文本
     */
    public static void showShort(CharSequence text, int defaultResId) {
        showShort(TextUtils.isEmpty(text) ? Utils.INSTANCE.getStringRes(defaultResId) : text);
    }
    ...
    private static void show(@StringRes int resId, int duration) {
        show(Utils.mContext.getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(Utils.mContext.getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(Utils.mContext);
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(Utils.mContext, spannableString, duration);
            } else {
                sToast = Toast.makeText(Utils.mContext, text, duration);
            }
        }
        View view = sToast.getView();
        if (view != null && bgResource != -1) {
            view.setBackgroundResource(bgResource);
        } else if (view != null && backgroundColor != DEFAULT_COLOR) {
